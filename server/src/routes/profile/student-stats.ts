...
const getStudentStatsWithoutPosition = async (githubId: string): Promise<StudentStats[]> => (
  await getRepository(Student)
    .createQueryBuilder('student')
...
export const getStudentStats = async (githubId: string) => {
  const studentStats = await getStudentStatsWithoutPosition(githubId);
