...
export class InterviewsController {
  ...
  // one day we can use just interview
  ...
  public async getInterviewFeedback(
    ...
    if (type !== 'stage-interview') {
      throw new BadRequestException('Only stage interviews are supported');
    }

    const feedback = await this.interviewFeedbackService.getStageInterviewFeedback(interviewId, user.githubId);

    if (!feedback) {
      throw new NotFoundException(`Feedback for interview ${interviewId} doesn't exist`);
    }

    return new InterviewFeedbackDto(feedback);
  }
