...
describe('list_my_students', () => {
  it('uses the mentorId from the session', async () => {
    ...
    const text = await runListMyStudents(ctx, { courseId: 5 });
  ...
  it('rejects when the user is not a mentor of the course', async () => {
    ...
    expect(await runListMyStudents(ctx, { courseId: 6 })).toBe('You are not a mentor of course 6.');
    expect(calls).toHaveLength(0);
  });

  it('surfaces API errors', async () => {
    const { ctx } = makeCtx({ get: () => apiFail(403), user: MENTOR_USER });
    expect(await runListMyStudents(ctx, { courseId: 5 })).toContain('Permission denied');
  });
});
