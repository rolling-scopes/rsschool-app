...
describe('get_my_cross_check_feedbacks', () => {
  it('fetches feedbacks for own solution', async () => {
    ...
    const text = await runGetMyCrossCheckFeedbacks(ctx, { courseId: 5, courseTaskId: 11 });
  ...
  it('surfaces API errors', async () => {
    ...
    expect(await runGetMyCrossCheckFeedbacks(ctx, { courseId: 5, courseTaskId: 11 })).toContain('Permission denied');
  });
});
