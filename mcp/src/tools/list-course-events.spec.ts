...
describe('list_course_events', () => {
  it('lists events of a course', async () => {
    ...
    const text = toText(await runListCourseEvents(ctx, { courseId: 5 }));
  ...
  it('reports empty events', async () => {
    ...
    expect(toText(await runListCourseEvents(ctx, { courseId: 5 }))).toBe('Course 5 has no events.');
  });

  it('surfaces API errors', async () => {
    const { ctx } = makeCtx({ get: () => apiFail(401) });
    expect(toText(await runListCourseEvents(ctx, { courseId: 5 }))).toContain('Authentication failed');
  });
});
