...
describe('complete_cross_check', () => {
  it('posts to the completion endpoint', async () => {
    ...
    const text = toText(await runCompleteCrossCheck(ctx, { courseId: 5, courseTaskId: 11 }));
  ...
  it('surfaces API errors', async () => {
    ...
    expect(toText(await runCompleteCrossCheck(ctx, { courseId: 5, courseTaskId: 11 }))).toContain('Permission denied');
  });
});
