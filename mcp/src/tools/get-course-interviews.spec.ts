...
describe('get_course_interviews', () => {
  it('lists interviews of a course', async () => {
    ...
    const text = await runGetCourseInterviews(ctx, { courseId: 5 });
  ...
  it('reports when there are no interviews', async () => {
    ...
    expect(await runGetCourseInterviews(ctx, { courseId: 5 })).toBe('Course 5 has no interviews.');
  });

  it('surfaces API errors', async () => {
    const { ctx } = makeCtx({ get: () => apiFail(403) });
    expect(await runGetCourseInterviews(ctx, { courseId: 5 })).toContain('Permission denied');
  });
});
