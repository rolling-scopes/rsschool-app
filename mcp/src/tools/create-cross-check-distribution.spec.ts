...
describe('create_cross_check_distribution', () => {
  it('posts to the distribution endpoint', async () => {
    ...
    const text = await runCreateCrossCheckDistribution(ctx, { courseId: 5, courseTaskId: 11 });
  ...
  it('surfaces API errors', async () => {
    ...
    expect(await runCreateCrossCheckDistribution(ctx, { courseId: 5, courseTaskId: 11 })).toContain(
      'Permission denied',
    );
  });
});
