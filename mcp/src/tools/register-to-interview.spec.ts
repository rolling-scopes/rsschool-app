...
describe('register_to_interview', () => {
  it('posts the registration', async () => {
    ...
    const text = await runRegisterToInterview(ctx, { courseId: 5, interviewId: 7 });
  ...
  it('surfaces API errors (registration not open)', async () => {
    ...
    expect(await runRegisterToInterview(ctx, { courseId: 5, interviewId: 7 })).toContain('not available yet');
  });
});
