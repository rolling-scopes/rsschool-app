...
describe('get_my_cross_check_result', () => {
  it('fetches the result using the session github id', async () => {
    ...
    const text = toText(await runGetMyCrossCheckResult(ctx, { courseId: 5, courseTaskId: 11 }));
  ...
  it('maps 404 to a not-reviewed-yet message', async () => {
    ...
    expect(toText(await runGetMyCrossCheckResult(ctx, { courseId: 5, courseTaskId: 11 }))).toContain(
