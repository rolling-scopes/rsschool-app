...
describe('get_student_summary', () => {
  it('fetches the summary and strips the mentor contact fields', async () => {
    ...
    const text = toText(await runGetStudentSummary(ctx, { courseId: 5, githubId: 'stud' }));
  ...
  it('maps 404 to a not-found message', async () => {
    ...
    expect(toText(await runGetStudentSummary(ctx, { courseId: 5, githubId: 'ghost' }))).toContain(
  ...
  it('surfaces other API errors', async () => {
    ...
    expect(toText(await runGetStudentSummary(ctx, { courseId: 5, githubId: 'stud' }))).toContain('Permission denied');
  });
});
