...
export const courseEventFieldsSchema = {
  dateTime: z.string().optional().describe('ISO date-time when the event starts'),
  endTime: z.string().optional().describe('ISO date-time when the event ends'),
  duration: z.number().optional().describe('Duration in minutes'),
  place: z.string().optional().describe('Place or platform of the event'),
  organizerId: z.number().int().optional().describe('User ID of the organizer'),
  special: z.string().optional().describe('Special tags, comma-separated'),
  broadcastUrl: z.string().optional().describe('Broadcast/stream URL'),
};
