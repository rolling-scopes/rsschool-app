...
describe('get_my_cross_check_review_stats', () => {
  it('fetches available review stats', async () => {
    ...
    const text = toText(await runGetMyCrossCheckReviewStats(ctx, { courseId: 5 }));
  ...
  it('reports when there are no open cross-checks', async () => {
    ...
    expect(toText(await runGetMyCrossCheckReviewStats(ctx, { courseId: 5 }))).toBe(
      'No open cross-check tasks right now.',
    );
  });

  it('surfaces API errors', async () => {
    const { ctx } = makeCtx({ get: () => apiFail(403) });
    expect(toText(await runGetMyCrossCheckReviewStats(ctx, { courseId: 5 }))).toContain('Permission denied');
  });
});
