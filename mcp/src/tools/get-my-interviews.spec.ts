...
describe('get_my_interviews', () => {
  it('fetches student interviews via students/me', async () => {
    ...
    const text = toText(await runGetMyInterviews(ctx, { courseId: 5 }));
  ...
  it('reports when there are no interviews', async () => {
    ...
    expect(toText(await runGetMyInterviews(ctx, { courseId: 5 }))).toBe('You have no interviews in this course.');
  });

  it('surfaces API errors', async () => {
    const { ctx } = makeCtx({ get: () => apiFail(401) });
    expect(toText(await runGetMyInterviews(ctx, { courseId: 5 }))).toContain('Authentication failed');
  });
});
