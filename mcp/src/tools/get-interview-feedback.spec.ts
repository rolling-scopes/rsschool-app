...
describe('get_interview_feedback', () => {
  it('fetches the stage-interview feedback form', async () => {
    ...
    const text = toText(await runGetInterviewFeedback(ctx, { courseId: 5, interviewId: 9 }));
  ...
  it('surfaces API errors', async () => {
    ...
    expect(toText(await runGetInterviewFeedback(ctx, { courseId: 5, interviewId: 9 }))).toContain('Permission denied');
  });
});
