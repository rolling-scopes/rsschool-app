...
describe('get_my_cross_check_assignments', () => {
  it('fetches assignments via the "me" alias', async () => {
    ...
    const text = await runGetMyCrossCheckAssignments(ctx, { courseId: 5, courseTaskId: 11 });
  ...
  it('reports when there are no assignments', async () => {
    ...
    expect(await runGetMyCrossCheckAssignments(ctx, { courseId: 5, courseTaskId: 11 })).toBe(
  ...
  it('surfaces API errors', async () => {
    ...
    expect(await runGetMyCrossCheckAssignments(ctx, { courseId: 5, courseTaskId: 11 })).toContain('Permission denied');
  });
});
