...
describe('get_mentor_dashboard', () => {
  it('fetches the dashboard for the session mentor', async () => {
    ...
    const text = await runGetMentorDashboard(ctx, { courseId: 5 });
  ...
  it('reports an empty dashboard', async () => {
    ...
    expect(await runGetMentorDashboard(ctx, { courseId: 5 })).toContain('dashboard is empty');
  });

  it('rejects when the user is not a mentor of the course', async () => {
    const { ctx } = makeCtx({ user: MENTOR_USER });
    expect(await runGetMentorDashboard(ctx, { courseId: 9 })).toBe('You are not a mentor of course 9.');
  });
