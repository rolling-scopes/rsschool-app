...
describe('list_course_mentors_details', () => {
  it('fetches mentor details', async () => {
    ...
    const text = await runListCourseMentorsDetails(ctx, { courseId: 5 });
  ...
  it('truncates to the limit', async () => {
    ...
    const text = await runListCourseMentorsDetails(ctx, { courseId: 5, limit: 20 });
  ...
  it('reports empty result', async () => {
    ...
    expect(await runListCourseMentorsDetails(ctx, { courseId: 5 })).toBe('Course 5 has no mentors.');
  });

  it('surfaces API errors', async () => {
    const { ctx } = makeCtx({ get: () => apiFail(403) });
    expect(await runListCourseMentorsDetails(ctx, { courseId: 5 })).toContain('Permission denied');
  });
});
