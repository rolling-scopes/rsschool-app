...
describe('get_my_mentor_interviews', () => {
  it('fetches mentor interviews via mentors/me', async () => {
    ...
    const text = await runGetMyMentorInterviews(ctx, { courseId: 5 });
  ...
  it('reports when there are no interviews', async () => {
    ...
    expect(await runGetMyMentorInterviews(ctx, { courseId: 5 })).toBe(
  ...
  it('surfaces API errors', async () => {
    ...
    expect(await runGetMyMentorInterviews(ctx, { courseId: 5 })).toContain('Permission denied');
  });
});
