...
describe('get_my_interview_students', () => {
  it('fetches interview students of the PAT mentor', async () => {
    ...
    const text = await runGetMyInterviewStudents(ctx, { courseId: 5 });
  ...
  it('reports when no students are assigned', async () => {
    ...
    expect(await runGetMyInterviewStudents(ctx, { courseId: 5 })).toBe(
  ...
  it('surfaces API errors', async () => {
    ...
    expect(await runGetMyInterviewStudents(ctx, { courseId: 5 })).toContain('Permission denied');
  });
});
