...
describe('get_my_score', () => {
  it('fetches the score using the session github id', async () => {
    ...
    const text = await runGetMyScore(ctx, { courseId: 5 });
  ...
  it('maps 404 to a not-a-student message', async () => {
    ...
    expect(await runGetMyScore(ctx, { courseId: 5 })).toContain('not a student of course 5');
  });

  it('surfaces other API errors', async () => {
    const { ctx } = makeCtx({ get: () => apiFail(401) });
    expect(await runGetMyScore(ctx, { courseId: 5 })).toContain('Authentication failed');
  });
});
