...
describe('issue_certificate', () => {
  it('posts to the certificate endpoint and reports success', async () => {
    ...
    const text = toText(await runIssueCertificate(ctx, { courseId: 5, studentGithubId: 'octo' }));
  ...
  it('maps 404 to a student-not-found message', async () => {
    ...
    expect(toText(await runIssueCertificate(ctx, { courseId: 5, studentGithubId: 'ghost' }))).toContain(
  ...
  it('maps 403 to a permission message', async () => {
    ...
    expect(toText(await runIssueCertificate(ctx, { courseId: 5, studentGithubId: 'octo' }))).toContain(
      'Permission denied',
    );
  });
  ...
  it('reports other statuses generically', async () => {
    ...
    expect(toText(await runIssueCertificate(ctx, { courseId: 5, studentGithubId: 'octo' }))).toContain('HTTP 500');
  });
});
