...
const MentorStatsCard = (props: Props) => {
  ...
  const showMentorStatsModalHandler = useCallback(
    (courseIndex: number) => {
  ...
  const countStudentsHandler = useCallback(
    (data: MentorStats[]) =>
  ...
  const shouldComponentUpdateHandler = useCallback(
    (_nextProps: Props, nextState: State) => !isEqual(nextState.isMentorStatsModalVisible, isMentorStatsModalVisible),
    [isMentorStatsModalVisible],
  );
