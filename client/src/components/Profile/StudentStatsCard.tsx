...
const StudentStatsCard = (props: Props) => {
  ...
  const shouldComponentUpdateHandler = useCallback(
    (_nextProps: Props, nextState: State) =>
      ...
      !isEqual(nextState.coursesProgress, coursesProgress),
    [isStudentStatsModalVisible, coursesProgress],
  );
  const showStudentStatsModalHandler = useCallback(
    (courseIndex: number) => {
  ...
  const showExpelConfirmationModalHandler = useCallback(
    (courseId: number) => {
      const content = (
        <>
          <Divider />
  ...
  const countScoredTasksHandler = useCallback(
    (tasks: { score: number }[]) => tasks.filter(({ score }) => score !== null).length,
    [],
  );
  const countCourseCompletionPercentageHandler = useCallback(
    (tasks: { score: number }[]) =>
