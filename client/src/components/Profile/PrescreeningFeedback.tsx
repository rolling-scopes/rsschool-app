...
export function PrescreeningFeedback({
  feedback,
  feedbackDecision,
}: {
  ...
  const steps = (feedback as { steps: InterviewFeedbackStepData[] }).steps.reduce(
    (map, step) => ({ ...map, [step.id]: step }),
    {} as Record<FeedbackStepId, InterviewFeedbackStepData>,
  );
  const { theory, practice, english, decision, intro } = steps;
  const isConducted = Object.values(FailedDecision).includes(feedbackDecision as FailedDecision);
