...
export function Feedback(props: StageFeedbackProps) {
  ...
  const { student, courseSummary, interviewFeedback, course, interviewId, type, interviewMaxScore } = props;

  const shouldFallbackToLegacy = !featureToggles.feedback || (interviewFeedback && !interviewFeedback.version);
  ...
  return (
    <Layout style={{ background: 'transparent', minHeight: '100vh' }}>
      ...
      <SubHeader completed={interviewFeedback?.isCompleted ?? false} />
      <StepContextProvider
        interviewFeedback={interviewFeedback}
        course={course}
        interviewId={interviewId}
        type={type}
        interviewMaxScore={interviewMaxScore}
      >
